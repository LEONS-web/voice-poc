...
async function transcribeWithWhisper(audio: Buffer, mimeType: string, signal?: AbortSignal): Promise<string> {
  const client = new OpenAI({ baseURL: env.stt.baseUrl, apiKey: env.stt.apiKey, timeout: 15000 });
 ...
 */
async function transcribeWithQwenAsr(audio: Buffer, mimeType: string, signal?: AbortSignal): Promise<string> {
  const client = new OpenAI({ baseURL: env.stt.baseUrl, apiKey: env.stt.apiKey, timeout: 15000 });
 ...
 * signal 用于客户端断开连接时中断上游请求，避免浪费调用额度。
 */
export async function transcribe(audio: Buffer, mimeType: string, signal?: AbortSignal): Promise<string> {
  ...
  if (env.stt.provider === 'qwen-asr') {
    return transcribeWithQwenAsr(audio, mimeType, signal);
  }
  return transcribeWithWhisper(audio, mimeType, signal);
}
